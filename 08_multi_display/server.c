...
int display_conn[10] = {0};
struct sockaddr_in display_addr[10];
...
void display(char *buffer) {
    ...
    for (int i = 0; i < 10; ++i) {
        if (display_conn[i]) {
            if (sendto(socket_fd, buffer, strlen(buffer) + 1, MSG_NOSIGNAL, (const struct sockaddr *) &display_addr[i], sizeof(display_addr[i])) <= 0) {
                display_conn[i] = 0;
                return;
            }
        }
